...
use crate::{
   process::ProcWork,
   vcf::{VcfRes, VcfCalc},
   alleles::{AllDesc, Trunc, LargeDeletion, get_large_deletions},
   depth::*,
   model::N_QUAL,
};

/// Estimate single base allele frequencies
pub(crate) fn estimate_single_base_freq(pw: &mut ProcWork, res: &mut Vec<VcfRes>, vc: &VcfCalc, ix: Option<usize>, iy: Option<usize>) {
   debug!("Estimating single base allele frequencies ({:?} - {:?})", ix, iy);
   ...
   for (i, dep) in counts[ix..iy].iter_mut().enumerate() {
      ...
      let opt_ins = if let Some(ins) = pw.depth.ins_hash.get(&x) {
         // Check if there is at least 1 insertion with the required number of observations
         if ins.hash.iter().map(|(_, &i)| &ins.alleles[i]).any(|ct| ct.cts[0] > 1 && ct.cts[1] > 1) {
            Some(ins)
         } else { None }
      } else { None };
      ...
      let vr = if let Some(ins_all) = opt_ins {
         ...
         add_allele(ref_ix);
         for b in (0..5).filter(|&x| x != ref_ix) { add_allele(b) }
         ...
         if log_enabled!(Trace) {
            ...
            for ar in vr.alleles.iter() {
               trace!("{}\t{}\t{}", &vc.all_desc[ar.ix], ar.res.freq, ar.res.lr_test);
            }
         }
         vr.adesc = None;
         vr
...
fn recalc_linear_block(pw: &mut ProcWork, start: usize, stop: usize, read_hash: &HashSet<usize>) {
   ...
   for (x, ct) in (start..=stop).zip(dep_cts[start..=stop].iter_mut()) {
      for c in ct.cts.iter_mut() { *c = [0; 2] }
      for qc in ct.qcts.iter_mut() { *qc = [0; N_QUAL] }
   ...
   for (_, da) in dalign.iter().enumerate().filter(|(k, _)| !read_hash.contains(k)) {
      da.process_allele(start, stop, |x, c, qual| {
         if (c as usize) % HIDX == BASES_INS {
            ...
            let (ix, q1) = da.ins_hash.get(&x).expect("Missing insertion index for read");
            ...
            if ix < 4 { // Don't put back residual deletions
               ct.cts[ix][st] += 1;
...
pub(crate) fn process_large_deletions(pw: &mut ProcWork, res: &mut Vec<VcfRes>, vc: &VcfCalc) -> Vec<LargeDeletion> {
   ...
   for (ix, vr) in res.iter().enumerate() {
      if is_del(vr) {
         if block_start.is_none() { block_start = Some(ix) }
   ...
   for(j, k) in blocks.drain(..) {
      ...
      if sz > vc.cfg.small_deletion_limit() {
         trace!("Large deletion block: {} - {}, len = {}", res[j].x + 1, res[k].x + 1, sz);
         let mut large_del = get_large_deletions(res, j, k, ref_len, pw.depth.dalign.as_mut().unwrap(), vc.cfg);
      ...
      if let Some((a, b)) = recalc {
         ...
         if b >= a {
            ...
            for (v1, v2) in res[a..=b].iter_mut().zip(nres.drain(..)) { *v1 = v2 }
         } else {
            estimate_single_base_freq(pw, &mut nres, vc, Some(a), None);
            for (v1, v2) in res[a..].iter_mut().zip(nres.drain(..)) { *v1 = v2 }
            estimate_single_base_freq(pw, &mut nres, vc, None, Some(b + 1));
            for (v1, v2) in res[..=b].iter_mut().zip(nres.drain(..)) { *v1 = v2 }
         }
      }
   };
   del_vec
