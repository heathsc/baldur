    ...
    pub fn est_freq(&self) -> anyhow::Result<Vec<f64>> {
        ...
        for it in 0..10000 {
            ...
            for (rd, ct) in self.read_dels.iter() {
                ...
                for (j, mut x) in rd.dset(1).iter().copied().enumerate() {
                    let mut i = 0;
                    while x != 0 {
                        if (x & 1) == 1 {
                            let ix = (j << 6) | i;
                ...
                if let Some(ix) = rd.obs_index() {
                    ...
                    cts[ix] += ct;
                    assert!(fq[ix] < 1.0 - excl_freq);
                    // lk += (fq[ix] / (1.0 - excl_freq)).ln() * ct
                } else {
                    ...
                    assert!(z - (1.0 - excl_freq) < 1.0e-12, "{z} {excl_freq}");
                    // lk += (z / (1.0 - excl_freq)).ln() * ct
                }
            }
            ...
            // eprintln!("{it}\t{lk}\t{delta}\t{z}\t{}\t{}", fq[0], fq[nd - 1]);
            if delta < 1.0e-8 {
                debug!(
